...
module.exports = {
    ...
    async execute(client, message) {
        fs.readFile(`Server/${message.message.member.guild.id.toString()}.json`, "utf8", async function (err, data) {
            ...
            json_data.memberjoin.member.aktiviert = false;
            fs.writeFile(`Server/${message.message.member.guild.id.toString()}.json`, JSON.stringify(json_data), () => {});

            const embed = new MessageEmbed()
                .setTitle("Set - Management - Member Join - Directly Message - Deactivate")
                .setDescription("Directly Message was successfully deactivated.")
                .setColor("#ff0000")
