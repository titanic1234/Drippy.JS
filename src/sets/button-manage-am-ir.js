    ...
    async execute (client, message) {
        fs.readFile(`Server/${message.message.member.guild.id.toString()}.json`, "utf8", async function (err, data) {
            ...
            const setManageEmbed = new MessageEmbed()
                .setTitle("Settings - Management - Automod - Ignored roles")
                .setDescription("What exactly do you want to set? Just click the button below.")
                .setColor("#003cff")
                .setFields(
                    {name: "Add a Ignored role", value: "Button: Add"},
                    {name: "Remove a Ignored role", value: "Button: Remove"},
                    {name: "Show all Ignored roles", value: "Button: Show"},
                    {
                        name: "\n\nINFO",
                ...
                "components": [
                    {
                        ...
                        "label": "Add",
                        "style": 1,
                        "custom_id": `setManageAMIRA:::${message.member.id}`
                    },
                    {
                        "type": 2,
                        "label": "Remove",
                        "style": 1,
                        "custom_id": `setManageAMIRR:::${message.member.id}`
                    },
                    {
                        "type": 2,
                        "label": "Show",
                        "style": 1,
                        "custom_id": `setManageAMIRS:::${message.member.id}`
                    },
                    {
                        "type": 2,
                        "label": "Back",
