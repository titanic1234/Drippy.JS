    ...
    async execute (client, message) {
        try {
            await fs.readFile(`Server/${message.message.member.guild.id.toString()}.json`, "utf8", async function (err, data) {
                ...
                var cmd = true;
                var channels = [];
                for (var p of json_data.automod.ignoredroles) {
                    channels.push(`<@&${p.toString()}> ID: ${p.toString()}`);
                }
                if (channels.length === 0) {
                    channels = ["There are no ignored roles yet.", "You can´t remove a role"];
                    cmd = false;
                }
                const setManageEmbed = new MessageEmbed()
                    .setTitle("Settings - Management - Automod - Ignored roles - Remove")
                    .setDescription("Please write now in the chat which role you want to remove. Either the ID or a mention. You can end the command by entering \"stop\".")
                    .setColor("#003cff")
                    .setFields(
                        {
                            name: "These are all ignored roles", value: "⠀\n" + channels.join("\n")
                        },
                        {
                            name: "\n\nINFO",
                    ...
                    "components": [
                        {
                            ...
                            "label": "Back",
                            "style": 4,
                            "custom_id": `setManageAMIRBack:::${message.member.id}`
                        }
                    ]
                };
                ...
                while (cmd) {
                    ...
                    if (role === null && save_role != null) {
                        ...
                        embed = new MessageEmbed()
                            .setTitle("Settings - Management - Automod - Ignored roles - Remove")
                            .setDescription("This role does not exist on this server!")
                            .setTimestamp()
                            .setColor("#ff0000")
                        ...
                        embed = new MessageEmbed()
                            .setTitle("Settings - Management - Automod - Ignored roles - Remove")
                            .setDescription("The role is not removed. Please specify a role again.")
                            .setTimestamp()
                            .setColor("#ff0000")
                    }
                    else if (msg.toString() === "y" && save_role != null) {
                        json_data.automod.ignoredroles = json_data.automod.ignoredroles.filter(object => {
                            return object !== save_role.id.toString();
                        });
                        ...
                        embed = new MessageEmbed()
                            .setTitle("Settings - Management - Automod - Ignored roles - Remove")
                            .setDescription("The role was removed! If you want to continue, just press the Continue button below.")
                            .setTimestamp()
                            .setColor("#00ff0d")
                        ...
                        if (json_data.automod.ignoredroles.includes(role.id.toString())) {
                            save_role = role;
                            embed = new MessageEmbed()
                                .setTitle("Settings - Management - Automod - Ignored roles - Remove")
                                .setDescription("Would you like to remove this role? If yes, please enter \"y\" now. If not, please enter \"n\" now.")
                                .setTimestamp()
                                .setColor("#ffec00")
                        } else {
                            embed = new MessageEmbed()
                                .setTitle("Settings - Management - Automod - Ignored roles - Remove")
                                .setDescription(":x:This role is not in the list!:x:")
                                .setTimestamp()
                                .setColor("#ff0000")
                        }
            ...
            console.log("Error in Remove Ignored channel: " + error.toString());
        }
    }
}