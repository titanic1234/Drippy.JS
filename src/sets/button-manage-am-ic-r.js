        ...
        try {
            await fs.readFile(`Server/${message.message.member.guild.id.toString()}.json`, "utf8", async function (err, data) {
                ...
                if (channels.length === 0) {
                    channels = ["There are no ignored channels yet.", "You can´t remove a channel"];
                    cmd = false;
                }
                const setManageEmbed = new MessageEmbed()
                    .setTitle("Settings - Management - Automod - Ignored channels - Remove")
                ...
                while (cmd) {
                    ...
                    if (channel === null && save_channel != null) {
                        ...
                        embed = new MessageEmbed()
                            .setTitle("Settings - Management - Automod - Ignored channels - Remove")
                            .setDescription("The channel is not removed. Please specify a channel again.")
                            .setTimestamp()
                            .setColor("#ff0000")
                    }
                    else if (msg.toString() === "y" && save_channel != null) {
                        json_data.automod.ignoredchannels = json_data.automod.ignoredchannels.filter(object => {
                            return object !== save_channel.id.toString();
                        });
                        ...
                        embed = new MessageEmbed()
                            .setTitle("Settings - Management - Automod - Ignored channels - Remove")
                            .setDescription("The channel was removed! If you want to continue, just press the Continue button below.")
                            .setTimestamp()
                            .setColor("#00ff0d")
                        ...
                        if (json_data.automod.ignoredchannels.includes(channel.id.toString())) {
                            ...
                            embed = new MessageEmbed()
                                .setTitle("Settings - Management - Automod - Ignored channels - Remove")
                                .setDescription("Would you like to remove this channel? If yes, please enter \"y\" now. If not, please enter \"n\" now.")
                                .setTimestamp()
                                .setColor("#ffec00")
                        } else {
                            embed = new MessageEmbed()
                                .setTitle("Settings - Management - Automod - Ignored channels - Remove")
                                .setDescription(":x:This channel is not in the list!:x:")
                                .setTimestamp()
                                .setColor("#ff0000")
                        }
