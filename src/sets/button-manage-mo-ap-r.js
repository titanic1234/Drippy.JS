//Set Button Manage Automod Ignorde Roles Remove

const { MessageActionRow, MessageButton, MessageEmbed, Permissions, MessageCollector} = require('discord.js');
const fs = require('fs');

module.exports = {
    name: "setbuttonmanagemoapr",
    description: "Settings",
    async execute (client, message) {
        try {
            await fs.readFile(`Server/${message.message.member.guild.id.toString()}.json`, "utf8", async function (err, data) {
                if (err) {
                    console.log(err);
                }


                var json_data = JSON.parse(data);




                if (message.message.guild.ownerId.toString() === message.message.member.id.toString()) {
                    return client.commands.get("permission_error").execute(client, message);
                }


                var cmd = true;
                var channels = [];
                for (var p of json_data.moderation.roles_admin) {
                    channels.push(`<@&${p.toString()}> ID: ${p.toString()}`);
                }
                if (channels.length === 0) {
                    channels = ["There are no admin roles yet.", "You can´t remove a role"];
                    cmd = false;
                }
                const setManageEmbed = new MessageEmbed()
                    .setTitle("Settings - Management - Moderation - Admin Roles - Remove")
                    .setDescription("Please write now in the chat which role you want to remove. Either the ID or a mention. You can end the command by entering \"stop\".")
                    .setColor("#003cff")
                    .setFields(
                        {
                            name: "These are all admin roles", value: "⠀\n" + channels.join("\n")
                        },
                        {
                            name: "\n\nINFO",
                            value: "More functions will be add channed. More detailed information on the individual functions can be obtained with #help"
                        }
                    )
                    .setTimestamp()
                    .setThumbnail(message.message.guild.iconURL())


                const buttons = {
                    "type": 1,
                    "components": [
                        {
                            "type": 2,
                            "label": "Back",
                            "style": 4,
                            "custom_id": `setManageMOAPBack:::${message.member.id}`
                        }
                    ]
                };

                await message.message.delete();
                await message.channel.send({embeds: [setManageEmbed], components: [buttons]});

                var role;
                var save_role = null;
                var msg2;
                var embed;
                var falsch = 0;


                while (cmd) {
                    embed = new MessageEmbed()
                        .setTitle("Settings - Management - Moderation - Admin Roles - Remove")
                        .setDescription("We could not understand this message. If you want to cancel the command, please enter \"stop\". ")
                        .setTimestamp()
                        .setColor("#ff0000")

                    const msg = await client.sets.get("awaitmessage").execute(client, message)

                    if (msg.author.id === client.id) continue;
                    if (msg.author.bot) continue;
                    if (message.user.id !== msg.author.id) continue;
                    msg2 = msg.content.split("<").join("").split("@").join("").split(">").join("").split("&").join("").toLowerCase();
                    role = msg.guild.roles.cache.get(msg2.toString());
                    if (role === null && save_role != null) {
                        falsch ++;
                        embed = new MessageEmbed()
                            .setTitle("Settings - Management - Moderation - Admin Roles - Remove")
                            .setDescription("This role does not exist on this server!")
                            .setTimestamp()
                            .setColor("#ff0000")

                    }
                    else if (msg.toString() === "n") {
                        save_role = null;
                        embed = new MessageEmbed()
                            .setTitle("Settings - Management - Moderation - Admin Roles - Remove")
                            .setDescription("The role is not removed. Please specify a role again.")
                            .setTimestamp()
                            .setColor("#ff0000")
                    }
                    else if (msg.toString() === "y" && save_role != null) {
                        json_data.moderation.roles_admin = json_data.moderation.roles_admin.filter(object => {
                            return object !== save_role.id.toString();
                        });
                        fs.writeFile(`Server/${message.message.member.guild.id.toString()}.json`, JSON.stringify(json_data), () => {
                        });
                        cmd = false;
                        embed = new MessageEmbed()
                            .setTitle("Settings - Management - Moderation - Admin Roles - Remove")
                            .setDescription("The role was removed! If you want to continue, just press the Continue button below.")
                            .setTimestamp()
                            .setColor("#00ff0d")


                        const button = {
                            "type": 1,
                            "components": [
                                {
                                    "type": 2,
                                    "label": "Continue",
                                    "style": 3,
                                    "custom_id": `setManageMOBack:::${message.member.id}`
                                }
                            ]
                        };
                        return msg.reply({embeds: [embed], components: [button]})
                    }
                    else if (msg.toString() === "stop") {
                        cmd = false;
                        embed = new MessageEmbed()
                            .setTitle("Settings - Management - Moderation - Admin Roles - Remove")
                            .setDescription("The command was stoped.")
                            .setTimestamp()
                            .setColor("#00a7ff")
                    }
                    else if (role != null && save_role === null) {
                        if (json_data.moderation.roles_admin.includes(role.id.toString())) {
                            save_role = role;
                            embed = new MessageEmbed()
                                .setTitle("Settings - Management - Moderation - Admin Roles - Remove")
                                .setDescription("Would you like to remove this role? If yes, please enter \"y\" now. If not, please enter \"n\" now.")
                                .setTimestamp()
                                .setColor("#ffec00")
                        } else {
                            embed = new MessageEmbed()
                                .setTitle("Settings - Management - Moderation - Admin Roles - Remove")
                                .setDescription(":x:This role is not in the list!:x:")
                                .setTimestamp()
                                .setColor("#ff0000")
                        }

                    }
                    else if (falsch >= 5) {
                        cmd = false;
                        embed = new MessageEmbed()
                            .setTitle("Settings - Management - Moderation - Admin Roles - Remove")
                            .setDescription("Too many unrecognisable messages were written. Please run the command again.")
                            .setTimestamp()
                            .setColor("#ff0000")
                    }
                    else {
                        falsch ++;
                    }

                    msg.reply({embeds: [embed]});

                }
            });
        }catch (error) {
            console.log("Error in Remove Ignored channel: " + error.toString());
        }
    }
}