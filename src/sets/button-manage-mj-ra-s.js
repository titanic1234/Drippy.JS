        ...
        try {
            await fs.readFile(`Server/${message.message.member.guild.id.toString()}.json`, "utf8", async function (err, data) {
                ...
                for (var p of json_data.memberjoin.roles) {
                    roles.push(`<@&${p.toString()}> ID: ${p.toString()}`);
                }
                if (roles.length === 0) {
                    roles = ["There are no auto roles yet."];
                }
                const setManageEmbed = new MessageEmbed()
                    .setTitle("Settings - Management - Member Join - Auto Role Add - Show")
                    .setDescription("⠀")
                    .setColor("#003cff")
                    .setFields(
                        {
            ...
            console.log("Error in Add Auto roles: " + error.toString());
        }
    }
}