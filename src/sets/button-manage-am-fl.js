//Set Button Manage Automod Forbidden Links

const { MessageActionRow, MessageButton, MessageEmbed, Permissions} = require('discord.js');
const fs = require('fs');

module.exports = {
    name: "setbuttonmanageamfl",
    ...
    async execute (client, message) {
        fs.readFile(`Server/${message.message.member.guild.id.toString()}.json`, "utf8", async function (err, data) {
            ...
            const setManageEmbed = new MessageEmbed()
                .setTitle("Settings - Management - Automod - Forbidden links")
                .setDescription("What exactly do you want to set? Just click the button below.")
                .setColor("#003cff")
                .setFields(
                    {name: "Add a Forbidden links", value: "Button: Add"},
                    {name: "Remove a Forbidden links", value: "Button: Remove"},
                    {name: "Show all Forbidden links", value: "Button: Show"},
                    {
                        name: "\n\nINFO",
                ...
                "components": [
                    {
                        ...
                        "label": "Add",
                        "style": 1,
                        "custom_id": `setManageAMFLA:::${message.member.id}`
                    },
                    {
                        "type": 2,
                        "label": "Remove",
                        "style": 1,
                        "custom_id": `setManageAMFLR:::${message.member.id}`
                    },
                    {
                        "type": 2,
                        "label": "Show",
                        "style": 1,
                        "custom_id": `setManageAMFLS:::${message.member.id}`
                    },
                    {
                        "type": 2,
                        "label": "Back",
