    ...
    async execute (client, message) {
        try {
            fs.readFile(`Server/${message.message.member.guild.id.toString()}.json`, "utf8", async function (err, data) {
                ...
                json_data.memberjoin.member.description = null;

                fs.writeFile(`Server/${message.message.member.guild.id.toString()}.json`, JSON.stringify(json_data), () => {});

                const embed = new MessageEmbed()
                    .setTitle("Settings - Management - Member Join - Directly Message - Reset")
                    .setDescription("The directly message text has been reset! If you want to continue, just press the Continue button below.")
                    .setTimestamp()
                    .setColor("#00ff0d")
