    ...
    async execute (client, message) {
        try {
            await fs.readFile(`Server/${message.message.member.guild.id.toString()}.json`, "utf8", async function (err, data) {
                ...
                var cmd = true;
                var channels = [];
                for (var p of json_data.moderation.roles_mod) {
                    channels.push(`<@&${p.toString()}> ID: ${p.toString()}`);
                }
                if (channels.length === 0) {
                    channels = ["There are no mod roles yet.", "You can´t remove a role"];
                    cmd = false;
                }
                const setManageEmbed = new MessageEmbed()
                    .setTitle("Settings - Management - Moderation - Mod Roles - Remove")
                    ...
                    "components": [
                        {
                            ...
                            "label": "Back",
                            "style": 4,
                            "custom_id": `setManageMOMPBack:::${message.member.id}`
                        }
                    ]
                };
                ...
                while (cmd) {
                    ...
                    if (role === null && save_role != null) {
                        ...
                        json_data.moderation.roles_mod = json_data.moderation.roles_mod.filter(object => {
                        ...
                        if (json_data.moderation.roles_mod.includes(role.id.toString())) {
                            save_role = role;
                            embed = new MessageEmbed()
                                .setTitle("Settings - Management - Moderation - Mod Roles - Remove")
