        ...
        try {
            await fs.readFile(`Server/${message.message.member.guild.id.toString()}.json`, "utf8", async function (err, data) {
                ...
                if (channels.length === 0) {
                    channels = ["There are no forbidden links yet."];
                }
                const setManageEmbed = new MessageEmbed()
                    .setTitle("Settings - Management - Automod - Forbidden links - Add")
                    .setDescription("Please write now in the chat which link you want to add. You can end the command by entering \"stop\".")
                    .setColor("#003cff")
                    .setFields(
                        {
                ...
                while (cmd) {
                    ...
                    if (msg.toString() === "n") {
                        ...
                        embed = new MessageEmbed()
                            .setTitle("Settings - Management - Automod - Forbidden links - Add")
                            .setDescription("The link is not added. Please specify a role again.")
                            .setTimestamp()
                            .setColor("#ff0000")
                    }
                    else if (msg.toString() === "y" && save_role != null) {
                        json_data.automod.links.push(save_role.toString());
                        fs.writeFile(`Server/${message.message.member.guild.id.toString()}.json`, JSON.stringify(json_data), () => {
                        });
                        cmd = false;
                        embed = new MessageEmbed()
                            .setTitle("Settings - Management - Automod - Forbidden links - Add")
                            .setDescription("The link was added! If you want to continue, just press the Continue button below.")
                            .setTimestamp()
                            .setColor("#00ff0d")
                        ...
                        if (!json_data.automod.links.includes(role.toString())) {
                            save_role = role;
                            embed = new MessageEmbed()
                                .setTitle("Settings - Management - Automod - Forbidden links - Add")
                                .setDescription("Would you like to add this link? If yes, please enter \"y\" now. If not, please enter \"n\" now.")
                                .setTimestamp()
                                .setColor("#ffec00")
                        } else {
                            embed = new MessageEmbed()
                                .setTitle("Settings - Management - Automod - Forbidden links - Add")
                                .setDescription(":x:This link has already been added to the list!:x:")
                                .setTimestamp()
                                .setColor("#ff0000")
                        }
            ...
            console.log("Error in Add Forbidden links: " + error.toString());
        }
    }
}